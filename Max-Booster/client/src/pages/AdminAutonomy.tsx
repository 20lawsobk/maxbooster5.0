...
export default function AdminAutonomy() {
  ...
  const { data: autopilotStatus, isLoading: autopilotLoading, error: autopilotError } = useQuery({ queryKey: autopilotKey });
  const { data: autonomousStatus, isLoading: autonomousLoading, error: autonomousError } = useQuery({ queryKey: autonomousKey });
  const { data: updatesStatus, isLoading: updatesLoading, error: updatesError } = useQuery({ queryKey: updatesKey });
  const { data: securityMetrics, isLoading: securityMetricsLoading, error: securityMetricsError } = useQuery({ queryKey: securityMetricsKey });
  const { data: securityThreats, isLoading: securityThreatsLoading, error: securityThreatsError } = useQuery({ queryKey: securityThreatsKey });
  ...
  if (autopilotLoading || autonomousLoading || updatesLoading || securityMetricsLoading || securityThreatsLoading) {
    return (
  ...
  if (autopilotError || autonomousError || updatesError || securityMetricsError || securityThreatsError) {
    return (
      <div className="min-h-screen flex items-center justify-center">
