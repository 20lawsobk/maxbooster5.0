...
function DashboardContent({ user }: { user: any }) {
  ...
  const hasPaidSubscription = user.subscriptionPlan && user.subscriptionPlan !== '' && user.subscriptionPlan !== 'trial';
  ...
  useEffect(() => {
    ...
    if (onboardingStatus && !onboardingStatus.hasCompletedOnboarding && !localStorage.getItem('onboardingSkipped')) {
  ...
  const quickActions = [
    ...
    },
    {
      ...
      description: hasPaidSubscription ? "Optimize your content for maximum reach" : "Upgrade to access AI features",
      icon: Brain,
      action: () => {
        if (!hasPaidSubscription) {
          toast({
            title: "Subscription Required",
            description: "AI features require an active paid subscription. Please upgrade your plan.",
            variant: "destructive"
          });
          return;
        }
      ...
      color: hasPaidSubscription ? "bg-purple-600 hover:bg-purple-700" : "bg-gray-400 cursor-not-allowed",
  ...
  return (
    <AppLayout>
          <div className="max-w-7xl mx-auto space-y-6" role="main" aria-label="Dashboard content">
            ...
            <header className="flex justify-between items-center" role="banner">
              <div>
                ...
                <p className="text-muted-foreground" role="status" aria-live="polite">
                  Hi {user?.username}! Start building your music career with our powerful AI-powered platform
                </p>
              </div>
              <nav className="flex items-center space-x-3" role="navigation" aria-label="Dashboard actions">
                <Button
                ...
                <Badge data-testid="badge-ai-enhanced" className="bg-green-100 text-green-800 border-green-300" role="status" aria-label="AI features enabled">
            ...
            {aiInsights && (
              <Card className="border-2 border-gradient-to-r from-blue-500 to-purple-600 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-950 dark:to-purple-950" role="region" aria-label="AI Performance Score">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right" role="status" aria-live="polite">
                      <div className="text-4xl font-bold text-green-600" aria-label={`Performance score: ${aiInsights?.performanceScore || 0} out of 100`}>
                        {aiInsights?.performanceScore || 0}/100
                      </div>
                      <div className="text-sm text-muted-foreground" role="status">
                        {aiInsights?.performanceScore >= 80 ? 'Excellent Performance' :
                         aiInsights?.performanceScore >= 60 ? 'Good Performance' :
                         aiInsights?.performanceScore >= 40 ? 'Room for Improvement' :
                         'Getting Started'}
                      </div>
                    </div>
                  </div>
            ...
            {/* Stats Grid */}
            <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6" role="region" aria-label="Performance statistics">
              {statsCards.map((stat, index) => {
                const Icon = stat.icon;
                return (
                  <Card 
                    key={index}
                    data-testid={(stat as any).testId || `card-stat-${stat.title.toLowerCase().replace(/\s+/g, '-')}`}
                  ...
                  >
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-sm font-medium text-muted-foreground mb-1" id={`stat-label-${index}`}>
                            {stat.title}
                          </p>
                          <p className={`text-2xl font-bold ${stat.color}`} aria-labelledby={`stat-label-${index}`} aria-describedby={`stat-change-${index}`}>
                            {stat.value}
                          </p>
                          <p className="text-xs text-green-600 flex items-center mt-1" id={`stat-change-${index}`}>
                            <ArrowUp className="w-3 h-3 mr-1" aria-label="Increase" />
                            {stat.change} from last month
                          </p>
                        </div>
                        <div className={`p-3 rounded-full ${stat.bgColor} ${stat.borderColor} border-2`} aria-hidden="true">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
              <TabsList className="grid w-full grid-cols-5">
                ...
                <TabsTrigger data-testid="tab-ai-insights" value="ai-insights">AI Insights</TabsTrigger>
                <TabsTrigger data-testid="tab-projects" value="projects">Projects</TabsTrigger>
                <TabsTrigger data-testid="tab-activity" value="activity">Activity</TabsTrigger>
              </TabsList>
