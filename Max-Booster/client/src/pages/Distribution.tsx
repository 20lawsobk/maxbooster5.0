...
export default function Distribution() {
  ...
  const analytics: DistributionAnalytics = comprehensiveAnalytics ? {
    ...
    platformBreakdown: comprehensiveAnalytics.streams?.byPlatform?.map((p) => ({
      platform: p.platform,
      streams: p.streams,
      earnings: p.revenue
    })) || []
  ...
  const { data: hyperFollowPages = [], isLoading: hyperFollowLoading } = useQuery<HyperFollowPage[]>({
  ...
  const initUploadMutation = useMutation({
    mutationFn: async ({ filename, totalSize }: { filename: string; totalSize: number }) => {
      const response = await apiRequest('POST', '/api/distribution/upload/init', { filename, totalSize });
      return response.json();
    },
    onSuccess: (data) => {
  ...
  const uploadChunkMutation = useMutation({
    mutationFn: async ({ sessionId, chunkIndex, chunkData, chunkHash }: any) => {
      const formData = new FormData();
      formData.append('chunk', new Blob([chunkData]));
      formData.append('chunkIndex', chunkIndex.toString());
      formData.append('chunkHash', chunkHash);
      
      const response = await apiRequest('POST', `/api/distribution/upload/${sessionId}/chunk`, formData);
      return response.json();
    },
  });
  ...
  const finalizeUploadMutation = useMutation({
    mutationFn: async (sessionId: string) => {
      const response = await apiRequest('POST', `/api/distribution/upload/${sessionId}/finalize`, {});
      return response.json();
    },
    onSuccess: () => {
      toast({
  ...
  const generateISRCMutation = useMutation({
    mutationFn: async ({ trackId, artist, title }: { trackId: string; artist: string; title: string }) => {
      const response = await apiRequest('POST', '/api/distribution/codes/isrc', { trackId, artist, title });
      return response.json();
    },
    onSuccess: (data) => {
  ...
  const generateUPCMutation = useMutation({
    mutationFn: async ({ releaseId, title }: { releaseId: string; title: string }) => {
      const response = await apiRequest('POST', '/api/distribution/codes/upc', { releaseId, title });
      return response.json();
    },
    onSuccess: (data) => {
  ...
  const submitToSpotifyMutation = useMutation({
    mutationFn: async ({ releaseId, credentials }: any) => {
      const response = await apiRequest('POST', '/api/distribution/platform/spotify', { releaseId, credentials });
      return response.json();
    },
    onSuccess: (data) => {
      toast({
  ...
  const submitToAppleMutation = useMutation({
    mutationFn: async ({ releaseId, credentials }: any) => {
      const response = await apiRequest('POST', '/api/distribution/platform/apple', { releaseId, credentials });
      return response.json();
    },
    onSuccess: (data) => {
      toast({
  ...
  const submitToYouTubeMutation = useMutation({
    mutationFn: async ({ releaseId, credentials }: any) => {
      const response = await apiRequest('POST', '/api/distribution/platform/youtube', { releaseId, credentials });
      return response.json();
    },
    onSuccess: (data) => {
      toast({
  ...
  const exportReportMutation = useMutation({
    mutationFn: async (format: 'csv' | 'pdf' = 'csv') => {
      if (format === 'pdf') {
        throw new Error('PDF export coming soon. Please use CSV format.');
      }
  ...
  const handleFileUpload = (files: FileList | null, type: 'audio' | 'artwork') => {
    ...
    if (type === 'audio') {
      const audioFiles = Array.from(files).filter(file => 
        file.type.includes('audio') || file.name.match(/\.(mp3|wav|flac|aac|ogg)$/i)
      );
          ...
          <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-green-600 dark:text-green-400 mt-1">
                        {analyticsGrowth?.earningsGrowth ? `+${analyticsGrowth.earningsGrowth.toFixed(1)}% this month` : 'No change'}
                      </p>
                    </div>
              ...
              <Card className="bg-gradient-to-br from-blue-50 to-cyan-100 dark:from-blue-900/20 dark:to-cyan-900/20 border-blue-200 dark:border-blue-800">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-blue-700 dark:text-blue-300">Total Streams</p>
                      <p className="text-3xl font-bold text-blue-900 dark:text-blue-100">
                        {(analytics?.totalStreams ?? 0).toLocaleString()}
                      </p>
                      <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                        {analyticsGrowth?.streamsGrowth ? `+${analyticsGrowth.streamsGrowth.toFixed(1)}% this month` : 'No change'}
                      </p>
                    </div>
              ...
              <Card className="bg-gradient-to-br from-purple-50 to-violet-100 dark:from-purple-900/20 dark:to-violet-900/20 border-purple-200 dark:border-purple-800">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-purple-700 dark:text-purple-300">Live Releases</p>
              ...
              <Card className="bg-gradient-to-br from-orange-50 to-amber-100 dark:from-orange-900/20 dark:to-amber-900/20 border-orange-200 dark:border-orange-800">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-orange-700 dark:text-orange-300">Platforms</p>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
              <TabsList className="grid w-full grid-cols-7 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                ...
                <TabsTrigger value="earnings" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white" data-testid="tab-earnings">
                  Earnings
                </TabsTrigger>
                <TabsTrigger value="hyperfollow" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white" data-testid="tab-hyperfollow">
                  HyperFollow
                </TabsTrigger>
                <TabsTrigger value="platforms" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white" data-testid="tab-platforms">
                  Platforms
                </TabsTrigger>
              </TabsList>
                      ...
                      {releasesLoading ? (
                        ...
                        <div className="space-y-3">
                          {releases.slice(0, 5).map((release: any) => (
                            <div key={release.id} className="flex items-center space-x-4 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                              <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-purple-600 rounded-lg flex items-center justify-center">
                                <Music className="w-6 h-6 text-white" />
                              </div>
                              <div className="flex-1">
                                <p className="font-medium text-gray-900 dark:text-white">{release.title}</p>
                                <p className="text-sm text-gray-500 dark:text-gray-400">{release.artistName}</p>
                              </div>
                    ...
                    <CardContent>
                      <div className="space-y-4">
                        {DISTRO_PLATFORMS.slice(0, 6).map((platform) => {
                          ...
                          return (
                            <div key={platform.id} className="flex items-center justify-between p-3 rounded-lg bg-gray-50 dark:bg-gray-700/50">
                              <div className="flex items-center space-x-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-gray-500 dark:text-gray-400">{platform.processingTime}</p>
                                </div>
                              </div>
              ...
              <TabsContent value="releases" className="space-y-6">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-gray-600 dark:text-gray-400">Manage and track your distributed music</p>
                  </div>
                  <Button 
                    onClick={() => setIsUploadOpen(true)}
                ...
                {releasesLoading ? (
                  ...
                  <Card className="bg-gradient-to-br from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 border-blue-200 dark:border-blue-800">
                    <CardContent className="p-12 text-center">
                      ...
                      <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">No Releases Yet</h3>
                      <p className="text-gray-600 dark:text-gray-400 mb-6 max-w-md mx-auto">
                        Start your music distribution journey by uploading your first release to 150+ platforms worldwide.
                      </p>
                      <Button 
                        onClick={() => setIsUploadOpen(true)}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {releases.map((release: any) => (
                      <Card key={release.id} className="group hover:shadow-xl transition-all duration-300 border border-gray-200 dark:border-gray-700">
                        <CardContent className="p-0">
                          ...
                          <div className="relative">
                            ...
                            <div className="absolute top-3 right-3">
                              <Badge 
                                variant={release.status === 'live' ? 'default' : release.status === 'processing' ? 'secondary' : 'destructive'}
                            ...
                            <div className="absolute bottom-3 left-3 right-3">
                              <div className="flex items-center justify-between text-white">
                                <div className="flex items-center space-x-2">
                                  ...
                                  <span className="text-sm font-medium">{release.totalStreams?.toLocaleString() || '0'}</span>
                                </div>
                                <div className="flex items-center space-x-2">
                                  <DollarSign className="w-4 h-4" />
                                  <span className="text-sm font-medium">${release.earnings?.toFixed(2) || '0.00'}</span>
                                </div>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="p-6">
                            <div className="mb-4">
                              ...
                              <p className="text-gray-600 dark:text-gray-400 text-sm mb-2">{release.artistName}</p>
                              <div className="flex items-center space-x-2">
                                <Badge variant="outline" className="text-xs">{release.releaseType}</Badge>
                                <Badge variant="outline" className="text-xs">{release.primaryGenre}</Badge>
                                {release.isExplicit && <Badge variant="destructive" className="text-xs">Explicit</Badge>}
                              </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4 mb-4">
                              <div className="text-center p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                                <p className="text-2xl font-bold text-blue-600">{release.tracksData?.length || 0}</p>
                                <p className="text-xs text-gray-500">Tracks</p>
                              </div>
                              <div className="text-center p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                                <p className="text-2xl font-bold text-green-600">{release.platforms?.length || 0}</p>
                              ...
                              <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                                <div className="flex items-center justify-between">
                                  <div className="flex items-center space-x-2">
                                    ...
                                    <span className="text-sm font-medium text-gray-900 dark:text-white">HyperFollow</span>
                                  </div>
                                  <Button 
                                    variant="ghost" 
                                    size="sm"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                            {analyticsGrowth?.streamsGrowth ? `${analyticsGrowth.streamsGrowth >= 0 ? '+' : ''}${analyticsGrowth.streamsGrowth.toFixed(1)}% vs last month` : 'No change'}
                          </p>
                        </div>
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-green-600 dark:text-green-400 mt-1">
                            {analyticsGrowth?.earningsGrowth ? `${analyticsGrowth.earningsGrowth >= 0 ? '+' : ''}${analyticsGrowth.earningsGrowth.toFixed(1)}% vs last month` : 'No change'}
                          </p>
                        </div>
                  ...
                  <Card className="bg-gradient-to-br from-purple-50 to-violet-100 dark:from-purple-900/20 dark:to-violet-900/20">
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-sm font-medium text-purple-700 dark:text-purple-300">Avg. Per Stream</p>
                          <p className="text-3xl font-bold text-purple-900 dark:text-purple-100">
                            ${((analytics?.totalEarnings ?? 0) / Math.max(analytics?.totalStreams ?? 1, 1)).toFixed(4)}
                          </p>
                          <p className="text-xs text-purple-600 dark:text-purple-400 mt-1">Industry average</p>
                        </div>
                  ...
                  <Card className="bg-gradient-to-br from-orange-50 to-amber-100 dark:from-orange-900/20 dark:to-amber-900/20">
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-sm font-medium text-orange-700 dark:text-orange-300">Active Platforms</p>
                          <p className="text-3xl font-bold text-orange-900 dark:text-orange-100">
                            {analytics?.platformBreakdown?.length ?? 0}
                          </p>
                          <p className="text-xs text-orange-600 dark:text-orange-400 mt-1">150+ available</p>
                        </div>
                    ...
                    <div className="space-y-4">
                      {analytics?.platformBreakdown && analytics.platformBreakdown.length > 0 ? (
                        analytics.platformBreakdown.map((platform, index) => (
                          <div key={index} className="flex items-center justify-between p-4 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition">
                            <div className="flex items-center space-x-4 flex-1">
                              ...
                              <div>
                                <p className="font-semibold text-gray-900 dark:text-white">{platform.platform}</p>
                            ...
                            <div className="text-right">
                              ...
                              <p className="text-xs text-gray-500">
                                ${((platform.earnings ?? 0) / Math.max(platform.streams ?? 1, 1)).toFixed(4)}/stream
                              </p>
                            </div>
                          </div>
                        ))
                        ...
                        <div className="text-center py-12">
                          ...
                          <p className="text-sm text-gray-400 mt-2">Your analytics will appear here once you have releases live</p>
                        </div>
                      )}
                    </div>
                  </CardContent>
                    ...
                    <CardContent>
                      <div className="space-y-3">
                        {streamingTrends.length > 0 ? (
                          streamingTrends.map((week: any, index: number) => (
                            <div key={index} className="flex items-center justify-between">
                              <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{week.date}</span>
                              <div className="flex items-center space-x-3">
                                <div className="w-32">
                                  <Progress value={(week.streams / (Math.max(...streamingTrends.map((w: any) => w.streams), 1))) * 100} className="h-2" />
                                </div>
                          ...
                          <p className="text-center text-gray-500 py-4">No streaming data available yet</p>
                        )}
                      </div>
                    </CardContent>
                  </Card>
                    ...
                    <CardContent>
                      <div className="space-y-3">
                        {releases.slice(0, 4).map((release, index) => (
                          <div key={release.id} className="flex items-center justify-between p-3 rounded-lg border border-gray-200 dark:border-gray-700">
                            <div className="flex items-center space-x-3">
                              ...
                              <div>
                                <p className="font-medium text-gray-900 dark:text-white">{release.title}</p>
                                <p className="text-xs text-gray-500">{release.totalStreams?.toLocaleString() ?? 0} streams</p>
                              </div>
                            </div>
                            <div className="text-right">
                      ...
                      {geographicData.length > 0 ? (
                        geographicData.map((region: any, index: number) => (
                          <div key={index} className="p-4 rounded-lg border border-gray-200 dark:border-gray-700">
                            <p className="font-semibold text-gray-900 dark:text-white mb-1">{region.region}</p>
                            <p className="text-2xl font-bold text-blue-600">{region.streams.toLocaleString()}</p>
                            <Progress value={region.percentage} className="h-1 mt-2" />
                            <p className="text-xs text-gray-500 mt-1">{region.percentage}% of total</p>
                          </div>
                        ))
                      ) : (
                        <p className="col-span-full text-center text-gray-500 py-4">No geographic data available yet</p>
                      )}
                    </div>
                  </CardContent>
                </Card>
              ...
              <TabsContent value="earnings" className="space-y-6">
                <div className="flex justify-between items-center">
                  <div>
                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Earnings & Royalties</h2>
                    <p className="text-gray-600 dark:text-gray-400">Track your revenue across all platforms</p>
                  </div>
                  <div className="flex space-x-2">
                    <Button 
                      variant="outline" 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-green-600 dark:text-green-400 mt-1">
                            {earningsBreakdown?.totalGrowth ? `+${earningsBreakdown.totalGrowth.toFixed(1)}% this month` : analyticsGrowth?.earningsGrowth ? `+${analyticsGrowth.earningsGrowth.toFixed(1)}% this month` : 'No change'}
                          </p>
                        </div>
                        <div className="p-3 bg-green-200 dark:bg-green-800 rounded-full">
                          <DollarSign className="w-6 h-6 text-green-700 dark:text-green-300" />
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                            {earningsBreakdown?.monthGrowth ? `+${earningsBreakdown.monthGrowth.toFixed(1)}% from last month` : 'No change'}
                          </p>
                        </div>
                        <div className="p-3 bg-blue-200 dark:bg-blue-800 rounded-full">
                  ...
                  <Card className="bg-gradient-to-br from-purple-50 to-violet-100 dark:from-purple-900/20 dark:to-violet-900/20 border-purple-200 dark:border-purple-800">
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-sm font-medium text-purple-700 dark:text-purple-300">Pending Payout</p>
                          <p className="text-3xl font-bold text-purple-900 dark:text-purple-100">
                            ${((earningsBreakdown?.pendingPayout ?? 0)).toFixed(2)}
                          </p>
                          <p className="text-xs text-purple-600 dark:text-purple-400 mt-1">
                            Next payout: {earningsBreakdown?.nextPayoutDate ? new Date(earningsBreakdown.nextPayoutDate).toLocaleDateString() : 'TBD'}
                          </p>
                        </div>
                        <div className="p-3 bg-purple-200 dark:bg-purple-800 rounded-full">
                  ...
                  <Card className="bg-gradient-to-br from-orange-50 to-amber-100 dark:from-orange-900/20 dark:to-amber-900/20 border-orange-200 dark:border-orange-800">
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-sm font-medium text-orange-700 dark:text-orange-300">Avg. per Stream</p>
                          <p className="text-3xl font-bold text-orange-900 dark:text-orange-100">
                            ${((analytics?.totalEarnings ?? 0) / (analytics?.totalStreams ?? 1) * 1000).toFixed(3)}
                          </p>
                          <p className="text-xs text-orange-600 dark:text-orange-400 mt-1">Per 1,000 streams</p>
                        </div>
                        <div className="p-3 bg-orange-200 dark:bg-orange-800 rounded-full">
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      {platformEarnings.length > 0 ? (
                        platformEarnings.map((platform: any) => {
                          const platformInfo = DISTRO_PLATFORMS.find(p => p.id === platform.platformId);
                          const IconComponent = platformInfo?.icon || Music;
                          
                          return (
                            <div key={platform.platformId} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                              <div className="flex items-center space-x-4">
                                <div className="w-10 h-10 bg-white dark:bg-gray-700 rounded-full flex items-center justify-between shadow-sm">
                                  <IconComponent className="w-5 h-5" style={{ color: platformInfo?.color }} />
                                </div>
                                <div>
                                  <p className="font-medium text-gray-900 dark:text-white">{platform.name}</p>
                                  <p className="text-sm text-gray-500 dark:text-gray-400">
                                    {(platform.streams ?? 0).toLocaleString()} streams • ${platform.perStreamRate?.toFixed(4) ?? '0.0000'} per stream
                                  </p>
                                </div>
                              </div>
                              <div className="text-right">
                                <p className="text-lg font-bold text-green-600">${(platform.earnings ?? 0).toFixed(2)}</p>
                                <p className="text-sm text-gray-500 dark:text-gray-400">
                        ...
                        <p className="text-center text-gray-500 py-4">No platform earnings data available yet</p>
                      )}
                    </div>
                  </CardContent>
                </Card>
                    ...
                    <div className="space-y-4">
                      {payoutHistory.length > 0 ? (
                        payoutHistory.map((payout: any, index: number) => (
                          <div key={payout.id || index} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                            <div className="flex items-center space-x-4">
                        ...
                        <p className="text-center text-gray-500 py-4">No payout history available yet</p>
                      )}
                    </div>
                  </CardContent>
                </Card>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          ...
                          <Select defaultValue="50">
                            ...
                            <SelectContent>
                              <SelectItem value="25" data-testid="select-payout-25">$25</SelectItem>
                              <SelectItem value="50" data-testid="select-payout-50">$50</SelectItem>
                              <SelectItem value="100" data-testid="select-payout-100">$100</SelectItem>
                              <SelectItem value="200" data-testid="select-payout-200">$200</SelectItem>
                            </SelectContent>
                          </Select>
                          <p className="text-xs text-gray-500 mt-1">
                        ...
                        <div>
                          ...
                          <Select defaultValue="monthly">
                            ...
                            <SelectContent>
                              <SelectItem value="weekly" data-testid="select-frequency-weekly">Weekly</SelectItem>
                              <SelectItem value="monthly" data-testid="select-frequency-monthly">Monthly</SelectItem>
                              <SelectItem value="quarterly" data-testid="select-frequency-quarterly">Quarterly</SelectItem>
                            </SelectContent>
                          </Select>
                          <p className="text-xs text-gray-500 mt-1">
                        ...
                        <div className="flex items-start space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-blue-700 dark:text-blue-300 mt-1">
                              All payments are processed securely through Stripe. Your banking information is encrypted and never stored on our servers.
                            </p>
                          </div>
                        </div>
                      </div>
              ...
              <TabsContent value="hyperfollow" className="space-y-6">
                <div className="flex justify-between items-center">
                  <div>
                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white">HyperFollow Pre-Save Pages</h2>
                    <p className="text-gray-600 dark:text-gray-400">Create landing pages to collect pre-saves and build your fanbase</p>
                  </div>
                  <Button 
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  <Card className="bg-gradient-to-br from-blue-50 to-cyan-100 dark:from-blue-900/20 dark:to-cyan-900/20 border-blue-200 dark:border-blue-800">
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-sm font-medium text-blue-700 dark:text-blue-300">Total Pre-Saves</p>
                          <p className="text-3xl font-bold text-blue-900 dark:text-blue-100">
                            {hyperFollowPages.reduce((sum, page) => sum + page.preSaves, 0).toLocaleString()}
                          </p>
                          <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                            {hyperFollowAnalytics?.preSavesGrowth ? `+${hyperFollowAnalytics.preSavesGrowth.toFixed(1)}% this month` : 'No change'}
                          </p>
                        </div>
                        <div className="p-3 bg-blue-200 dark:bg-blue-800 rounded-full">
                  ...
                  <Card className="bg-gradient-to-br from-green-50 to-emerald-100 dark:from-green-900/20 dark:to-emerald-900/20 border-green-200 dark:border-green-800">
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-sm font-medium text-green-700 dark:text-green-300">Page Views</p>
                          <p className="text-3xl font-bold text-green-900 dark:text-green-100">
                            {hyperFollowPages.reduce((sum, page) => sum + page.pageViews, 0).toLocaleString()}
                          </p>
                          <p className="text-xs text-green-600 dark:text-green-400 mt-1">
                            {hyperFollowAnalytics?.viewsGrowth ? `+${hyperFollowAnalytics.viewsGrowth.toFixed(1)}% this month` : 'No change'}
                          </p>
                        </div>
                        <div className="p-3 bg-green-200 dark:bg-green-800 rounded-full">
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-purple-600 dark:text-purple-400 mt-1">
                            {hyperFollowAnalytics?.conversionGrowth ? `+${hyperFollowAnalytics.conversionGrowth.toFixed(1)}% this month` : 'No change'}
                          </p>
                        </div>
                        <div className="p-3 bg-purple-200 dark:bg-purple-800 rounded-full">
                  ...
                  <Card className="bg-gradient-to-br from-orange-50 to-amber-100 dark:from-orange-900/20 dark:to-amber-900/20 border-orange-200 dark:border-orange-800">
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-sm font-medium text-orange-700 dark:text-orange-300">Active Pages</p>
                          <p className="text-3xl font-bold text-orange-900 dark:text-orange-100">
                            {hyperFollowPages.filter(page => page.isActive).length}
                          </p>
                          <p className="text-xs text-orange-600 dark:text-orange-400 mt-1">Live campaigns</p>
                        </div>
                        <div className="p-3 bg-orange-200 dark:bg-orange-800 rounded-full">
                ...
                {hyperFollowLoading ? (
                  ...
                  <Card className="bg-gradient-to-br from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 border-blue-200 dark:border-blue-800">
                    <CardContent className="p-12 text-center">
                      ...
                      <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">No HyperFollow Pages Yet</h3>
                      <p className="text-gray-600 dark:text-gray-400 mb-6 max-w-md mx-auto">
                        Create your first HyperFollow page to start collecting pre-saves and building your fanbase before your release goes live.
                      </p>
                      <Button 
                        onClick={() => setIsHyperFollowOpen(true)}
                        className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 px-8 py-3"
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                      {DISTRO_PLATFORMS.map((platform) => {
                        const IconComponent = platform.icon;
                        return (
                          <div key={platform.id} className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                            <div className="flex items-center space-x-3 mb-3">
                              <div className="w-8 h-8 bg-white dark:bg-gray-800 rounded-full flex items-center justify-center shadow-sm">
                                <IconComponent className="w-4 h-4" />
                              </div>
                              <div>
                                <p className="font-medium text-sm text-gray-900 dark:text-white">{platform.name}</p>
              ...
              <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                ...
                <div className="mt-6">
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) => setUploadForm(prev => ({ ...prev, artistName: e.target.value }))}
                          />
                        </div>
                        <div>
                          ...
                          <Select 
                            ...
                            onValueChange={(value) => 
                              setUploadForm(prev => ({ ...prev, releaseType: value as 'single' | 'album' | 'EP' }))
                            }
                          >
                            <SelectTrigger>
                              <SelectValue />
                            ...
                            <SelectContent>
                              <SelectItem value="single" data-testid="select-type-single">Single</SelectItem>
                              <SelectItem value="EP" data-testid="select-type-ep">EP</SelectItem>
                              <SelectItem value="album" data-testid="select-type-album">Album</SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
                        <div>
                          ...
                          <Select 
                            ...
                            onValueChange={(value) => setUploadForm(prev => ({ ...prev, primaryGenre: value }))}
                          >
                            <SelectTrigger>
                            ...
                            <SelectContent>
                              {GENRES.map((genre) => (
                                <SelectItem key={genre} value={genre} data-testid={`select-genre-${genre.toLowerCase().replace(/\s+/g, '-')}`}>{genre}</SelectItem>
                              ))}
                            </SelectContent>
                          </Select>
                        </div>
                        <div>
                          ...
                          >
                            ...
                            <SelectContent>
                              <SelectItem value="English" data-testid="select-language-english">English</SelectItem>
                              <SelectItem value="Spanish" data-testid="select-language-spanish">Spanish</SelectItem>
                              <SelectItem value="French" data-testid="select-language-french">French</SelectItem>
                              <SelectItem value="German" data-testid="select-language-german">German</SelectItem>
                              <SelectItem value="Italian" data-testid="select-language-italian">Italian</SelectItem>
                              <SelectItem value="Portuguese" data-testid="select-language-portuguese">Portuguese</SelectItem>
                              <SelectItem value="Japanese" data-testid="select-language-japanese">Japanese</SelectItem>
                              <SelectItem value="Korean" data-testid="select-language-korean">Korean</SelectItem>
                              <SelectItem value="Mandarin" data-testid="select-language-mandarin">Mandarin</SelectItem>
                              <SelectItem value="Other" data-testid="select-language-other">Other</SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
                        <div>
                      ...
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => setUploadForm(prev => ({ ...prev, isExplicit: checked as boolean }))}
                        />
                            ...
                            {uploadForm.audioFiles.map((file, index) => (
                              <div key={index} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                                ...
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() => {
                                    const newFiles = uploadForm.audioFiles.filter((_, i) => i !== index);
                                    const newTracks = uploadForm.tracks.filter((_, i) => i !== index);
                                    setUploadForm(prev => ({ ...prev, audioFiles: newFiles, tracks: newTracks }));
                                  }}
                          ...
                          {uploadForm.albumArt ? (
                            <div className="flex items-center space-x-4">
                              ...
                              <div className="flex-1">
                                ...
                                <p className="text-sm text-gray-500">{(uploadForm.albumArt.size / 1024 / 1024).toFixed(2)} MB</p>
                              </div>
                              <Button
                                variant="ghost"
                                size="sm"
                            ...
                            <div className="text-center">
                              ...
                              <p className="text-gray-600 dark:text-gray-300 mb-2">Upload album artwork</p>
                              <p className="text-sm text-gray-500 mb-4">3000x3000 pixels recommended (JPG, PNG)</p>
                              <Button
                                type="button"
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <div className="flex items-center space-x-2">
                            <Checkbox
                              ...
                              onCheckedChange={(checked) => setUploadForm(prev => ({ ...prev, isScheduled: checked as boolean }))}
                            />
                          ...
                          {uploadForm.isScheduled && (
                            <Input
                              type="date"
                              value={uploadForm.scheduledDate ? uploadForm.scheduledDate.toISOString().split('T')[0] : ''}
                              onChange={(e) => setUploadForm(prev => ({ ...prev, scheduledDate: new Date(e.target.value) }))}
                              className="mt-2"
                        ...
                        <div>
                          ...
                          <Select
                            ...
                            onValueChange={(value) => setUploadForm(prev => ({ ...prev, iTunesPricing: value }))}
                          >
                            <SelectTrigger>
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                              <SelectItem value="standard" data-testid="select-pricing-standard">Standard Pricing</SelectItem>
                              <SelectItem value="premium" data-testid="select-pricing-premium">Premium Pricing (+30%)</SelectItem>
                              <SelectItem value="budget" data-testid="select-pricing-budget">Budget Pricing (-20%)</SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center space-x-2">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) => setUploadForm(prev => ({ ...prev, leaveALegacy: checked as boolean }))}
                          />
                        ...
                        {uploadForm.leaveALegacy && (
                          <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
                            <p className="text-sm text-blue-700 dark:text-blue-300">
                              Leave a Legacy keeps your music online forever, even if you stop paying for DistroKid.
                              Your music will never be taken down from stores.
                            </p>
                          </div>
                        )}
                      </div>
                  ...
                  {currentStep === 5 && (
                    <div className="space-y-6">
                      ...
                      <div className="bg-gray-50 dark:bg-gray-800 p-6 rounded-lg space-y-4">
                        ...
                        <div>
                          <Label className="text-sm font-medium text-gray-500">Tracks ({uploadForm.audioFiles.length})</Label>
                          <div className="space-y-2 mt-2">
                            {uploadForm.tracks.map((track, index) => (
                              <div key={index} className="flex items-center space-x-3 p-2 bg-white dark:bg-gray-700 rounded">
                                <span className="text-sm font-mono text-gray-500">{index + 1}.</span>
                                <span className="font-medium">{track.title}</span>
                                {track.explicit && <Badge variant="destructive" className="text-xs">Explicit</Badge>}
                              </div>
                            ))}
                          </div>
                        </div>
                        ...
                        <div>
                          <Label className="text-sm font-medium text-gray-500">Platforms ({uploadForm.selectedPlatforms.length})</Label>
                        ...
                        <div className="flex items-start space-x-3">
                          ...
                          <div>
                            ...
                            <ul className="text-sm text-yellow-700 dark:text-yellow-300 mt-2 space-y-1">
                              <li>• Make sure all track titles and artist names are spelled correctly</li>
                              <li>• Verify your album artwork meets platform requirements (3000x3000px)</li>
                              <li>• Double-check that all collaborators are properly credited</li>
                              <li>• Review your platform selection - you can't change this after submission</li>
                            </ul>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="flex justify-between mt-8">
                    ...
                    >
                      {uploadReleaseMutation.isPending ? 'Uploading...' : currentStep === 5 ? 'Submit Release' : 'Next'}
                    </Button>
                  </div>
                </div>
              </DialogContent>
