  ...
  return (
    <AppLayout>
      <div className="p-6" role="main" aria-label="Projects management">
          ...
          <header className="flex justify-between items-center mb-6" role="banner">
            ...
            <Dialog open={isUploadOpen} onOpenChange={setIsUploadOpen}>
              <DialogTrigger asChild>
                <Button className="gradient-bg" data-testid="button-upload-project" aria-label="Upload new project">
              ...
              <DialogContent className="max-w-md">
                ...
                <form onSubmit={handleUpdateProject} className="space-y-4">
                  ...
                  <div>
                    ...
                    <Select value={editForm.genre} onValueChange={(value) => setEditForm(prev => ({ ...prev, genre: value }))}>
                      <SelectTrigger>
                        <SelectValue placeholder="Select genre" />
                      </SelectTrigger>
                      <SelectContent>
          ...
          {/* Projects Grid */}
          {projectsLoading ? (
            <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" role="region" aria-label="Loading projects" aria-busy="true">
              {[...Array(6)].map((_, i) => (
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" role="region" aria-label="Projects grid">
                    ...
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => handlePlayProject(project)} data-testid={`button-play-${project.id}`}>
                            {currentlyPlaying === project.id ? <Pause className="h-4 w-4 mr-2" /> : <Play className="h-4 w-4 mr-2" />}
                            {currentlyPlaying === project.id ? 'Pause' : 'Play'}
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleEdit(project)} data-testid={`button-edit-${project.id}`}>
                            <Edit className="h-4 w-4 mr-2" />
                            Edit
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => setLocation(`/analytics?project=${project.id}`)} data-testid={`button-analytics-${project.id}`}>
