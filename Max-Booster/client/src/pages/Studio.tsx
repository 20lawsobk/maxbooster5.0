...
import { useAudioPlayer, type Track as AudioPlayerTrack, getCPUUsage } from '@/hooks/useAudioPlayer';
...
import {
  Upload, Save, Brain, Sparkles, Wand2, AudioWaveform, Plus, Minus,
  Music, Piano, Zap, FileAudio, FileText, X, ChevronDown, MonitorSpeaker,
  Download, Layers, Sliders, Activity, Loader2, Maximize, Minimize, RotateCw, Share2, Link2
} from 'lucide-react';
...
export default function Studio() {
  ...
  const {
    ...
    onStateChange: (type, state) => {
      console.log(`[Studio] ${type} state changed to: ${state}`);
    },
    onError: (type, error) => {
      console.error(`[Studio] ${type} error:`, error);
    },
    onSuccess: (type, data) => {
      console.log(`[Studio] ${type} success:`, data);
      if (type === 'ai-mix' || type === 'ai-master') {
        queryClient.invalidateQueries({ queryKey: ['/api/studio/projects', selectedProject?.id, 'tracks'] });
      }
  ...
  const createProjectMutation = useMutation({
    mutationFn: async (title: string) => {
      const res = await apiRequest('POST', '/api/studio/projects', { title, bpm: 120, status: 'draft' });
  ...
  const updateTrackMutation = useMutation({
    mutationFn: async ({ trackId, updates }: { trackId: string; updates: Partial<StudioTrack> }) => {
  ...
  const deleteTrackMutation = useMutation({
    mutationFn: async (trackId: string) => {
      if (!selectedProject) throw new Error('No project selected');
      return await apiRequest('DELETE', `/api/studio/tracks/${trackId}`, { projectId: selectedProject.id });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/studio/projects', selectedProject?.id, 'tracks'] });
  ...
  const createMixBusMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/studio/projects', selectedProject?.id, 'mix-busses'] });
  ...
  const reorderTracksMutation = useMutation({
    mutationFn: async ({ trackIds }: { trackIds: string[] }) => {
      if (!selectedProject) throw new Error('No project selected');
      return await apiRequest('PATCH', `/api/studio/projects/${selectedProject.id}/tracks/reorder`, {
        trackIds,
      });
    },
    onSuccess: () => {
  ...
  const exportProjectMutation = useMutation({
    mutationFn: async () => {
      ...
      const response = await apiRequest('POST', `/api/studio/export`, { 
        ...
        startTime: 0,
        endTime: null,
        trackIds: exportType === 'tracks' ? tracks.filter(t => selectedTrack === t.id).map(t => t.id) : undefined,
        metadata: {
          artist: '',
      ...
      const tracksWithClips = await Promise.all(
        tracks.map(async (track) => {
          try {
            const clipsResponse = await apiRequest('GET', `/api/studio/tracks/${track.id}/audio-clips`);
            const clipsData = await clipsResponse.json();
            const clips = clipsData.clips || clipsData || [];
            return { track, clips };
          } catch (error) {
      ...
      const exportTracks = tracksWithClips.map(({ track, clips }) => {
        let audioUrl = '';
        if (clips.length > 0) {
      ...
      const exportResult = await exportAudio({
        ...
        if (progress.stage === 'loading') {
          ...
          toast({ title: progress.message, description: 'Rendering with OfflineAudioContext...' });
      ...
      toast({ title: 'Uploading rendered audio...', description: 'Sending to server for format conversion...' });
        ...
        const checkStatus = setInterval(async () => {
          try {
            ...
            if (status.status === 'completed') {
              ...
              setExportJobId(null);
              setExportProgress(0);
              toast({ title: `Export completed as ${exportFormat.toUpperCase()}`, description: 'Download starting...' });
              window.open(`/api/studio/export/${data.jobId}/download`, '_blank');
            } else if (status.status === 'failed') {
              clearInterval(checkStatus);
              setExportJobId(null);
              setExportProgress(0);
              toast({ title: 'Export failed', description: status.error || 'Unknown error', variant: 'destructive' });
            }
          } catch (error) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const usage = getCPUUsage(audioPlayer.context);
      setCpuUsage(usage);
  ...
  useEffect(() => {
    ...
    if (projectIdFromUrl && projects.length > 0) {
      const project = projects.find((p: any) => p.id === projectIdFromUrl || p.id.toString() === projectIdFromUrl);
  ...
  useEffect(() => {
    ...
    const loadTracksIntoController = async () => {
      try {
        ...
        console.error('Error loading tracks into controller:', error);
        toast({ 
    ...
    const handleInputMonitoring = async () => {
      for (const track of displayTracks) {
        ...
        if (shouldMonitor && !isMonitoring) {
          try {
            const monitoringData = await multiTrackRecorder.startInputMonitoring(track.id, track.name);
            if (monitoringData) {
              inputMonitoringRefs.current.set(track.id, monitoringData);
            }
          } catch (error) {
            console.error(`Failed to start input monitoring for track ${track.id}:`, error);
          }
  ...
  const handleTrackUpdate = useCallback(async (trackId: string, updates: Partial<StudioTrack>) => {
    try {
  ...
  const handleVolumeChange = useCallback(async (trackId: string, volume: number) => {
    try {
  ...
  const handlePanChange = useCallback(async (trackId: string, pan: number) => {
    try {
      ...
      console.error('Error changing pan:', error);
    }
  }, [controller]);
  ...
  const handleTrackNameChange = useCallback(async (trackId: string, name: string) => {
    try {
      await controller.updateTrack(trackId, { name });
    } catch (error) {
      console.error('Error changing track name:', error);
    }
  }, [controller]);

  const handleReorderTracks = useCallback((oldIndex: number, newIndex: number) => {
    if (!selectedProject) return;
  ...
  const handleSkipForward = useCallback(() => { 
    const newTime = Math.min(audioPlayer.state?.duration ?? 60, controller.transport.currentTime + 5); 
    controller.seek(newTime);
  ...
  const handleTimelineClick = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const duration = audioPlayer.state?.duration ?? 60;
    const newTime = (clickX / timelineWidth) * duration;
    controller.seek(newTime);
  }, [audioPlayer, controller]);

  const handleClipUpdate = useCallback(async (trackId: string, clipId: string, updates: { startTime?: number; duration?: number }) => {
    try {
      ...
      toast({ title: 'Failed to update clip', description: error.message, variant: 'destructive' });
    }
  ...
  const incrementTempo = useCallback(() => controller.setTempo(Math.min(240, controller.transport.tempo + 1)), [controller]);
  const decrementTempo = useCallback(() => controller.setTempo(Math.max(40, controller.transport.tempo - 1)), [controller]);
  ...
  const handleStartRecording = useCallback(async () => {
    ...
    if (armedTracks.length === 0) {
      toast({ title: 'No tracks armed for recording', description: 'Arm at least one track', variant: 'destructive' });
      return;
    }
    try {
      controller.startRecording();
      const { sessionId } = await multiTrackRecorder.startRecording(armedTracks.map(t => ({ id: t.id, name: t.name })), { startPosition: controller.transport.currentTime, takeNumber: 1, isLoopRecording: controller.transport.loopEnabled });
      const interval = window.setInterval(() => setRecordingDuration(multiTrackRecorder.duration), 100);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (lyricsContent !== (projectLyrics?.content || '')) {
        saveLyricsMutation.mutate({ projectId: selectedProject.id.toString(), content: lyricsContent });
      }
    }, 1000);
  ...
  const addGeneratedTrackMutation = useMutation({
    mutationFn: async ({ audioFilePath, parameters }: { audioFilePath: string; parameters: any }) => {
      if (!selectedProject) throw new Error('No project selected');
  ...
  const handleAddGeneration = useCallback((audioFilePath: string, parameters: any) => {
  ...
  return (
    <AppLayout noPadding={!isDataLoading}>
      {isDataLoading ? (
        <div className="flex items-center justify-center h-full" role="status" aria-label="Loading studio">
          <Loader2 className="h-8 w-8 animate-spin text-gray-400" />
          <span className="sr-only">Loading studio components...</span>
        </div>
      ) : (
      <div ref={dawContainerRef} className="h-full flex flex-col studio-container" role="application" aria-label="Digital Audio Workstation">
        <LayoutGrid
          topBar={
            <StudioTopBar
              ...
              <div className="border-b" style={{ borderColor: 'var(--studio-border)', backgroundColor: 'var(--studio-bg-medium)' }}>
                <div className="overflow-x-auto scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800">
                  <div className="flex items-center gap-2 min-w-max px-4 py-2">
                    ...
                    />
                    <Button size="sm" variant="ghost" className="h-8 text-purple-400 hover:text-purple-300 hover:bg-purple-900/20" onClick={() => setIsAIMixing(true)} data-testid="button-ai-mix">
                      <Brain className="h-4 w-4 mr-1" />AI Mix
                    </Button>
                    <Button size="sm" variant="ghost" className="h-8 text-blue-400 hover:text-blue-300 hover:bg-blue-900/20" onClick={() => setIsAIMastering(true)} data-testid="button-ai-master">
                      <Sparkles className="h-4 w-4 mr-1" />AI Master
                    </Button>
                    <Button size="sm" variant="ghost" className="h-8 text-amber-400 hover:text-amber-300 hover:bg-amber-900/20" onClick={() => setShowAIGeneratorDialog(true)} data-testid="button-ai-generator">
                      <Wand2 className="h-4 w-4 mr-1" />AI Generator
                    </Button>
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button 
                            ...
                            disabled={!selectedProject || tracks.length === 0 || audioAnalysis.currentState === 'requesting' || audioAnalysis.currentState === 'processing'}
                            data-testid="button-analyze-audio"
                          >
                            {audioAnalysis.currentState === 'requesting' || audioAnalysis.currentState === 'processing' ? (
                              <><Loader2 className="h-4 w-4 mr-1 animate-spin" />Analyzing...</>
                            ) : (
                              <><Activity className="h-4 w-4 mr-1" />Analyze</>
                            )}
                          </Button>
                        </TooltipTrigger>
                        <TooltipContent>
                    ...
                    <Button size="sm" variant={showLyricsPanel ? 'secondary' : 'ghost'} className="h-8" onClick={() => setShowLyricsPanel(!showLyricsPanel)} data-testid="button-toggle-lyrics">
                      <FileText className="h-4 w-4 mr-1" />Lyrics
                    </Button>
                    <Separator orientation="vertical" className="h-6 bg-gray-600" />
                    {lastSaved && (
                      <div className="text-xs text-gray-400" data-testid="text-autosave-indicator">
                        {isSaving ? <span className="text-blue-400">💾 Saving...</span> : <span>💾 Saved {new Date().getTime() - lastSaved.getTime() < 60000 ? 'just now' : `${Math.floor((new Date().getTime() - lastSaved.getTime()) / 60000)}m ago`}</span>}
                      </div>
                    )}
                    ...
                    <Button size="sm" variant="ghost" className="h-8" onClick={() => setShowDistributionDialog(true)} disabled={!selectedProject} data-testid="button-open-distribution">
                      <FileText className="h-4 w-4 mr-1" />Distribution
                    </Button>
                    <Button size="sm" variant="ghost" className="h-8" onClick={handleShareLink} disabled={!selectedProject} data-testid="button-share-link">
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p>{isFullscreen ? 'Exit fullscreen (ESC)' : 'Enter fullscreen mode'}</p>
                        </TooltipContent>
                      </Tooltip>
                    </TooltipProvider>
                    <Separator orientation="vertical" className="h-6 bg-gray-600" />
                    <Button size="sm" variant={view === 'arrangement' ? 'secondary' : 'ghost'} className="h-8" onClick={() => setView('arrangement')} data-testid="button-view-arrangement">
                      <Layers className="h-4 w-4 mr-1" />Arrangement
                    </Button>
                    <Button size="sm" variant={view === 'mixer' ? 'secondary' : 'ghost'} className="h-8" onClick={() => setView('mixer')} data-testid="button-view-mixer">
                      <Sliders className="h-4 w-4 mr-1" />Mixer
                    </Button>
                  </div>
                </div>
              </div>
                ...
                {!selectedProject ? (
                  <div className="h-full flex items-center justify-center">
                    <div className="text-center max-w-md">
                      ...
                      <p className="text-sm text-gray-400 mb-4">Create a new project or select an existing one from the dropdown above to get started</p>
                    </div>
                  </div>
                ) : (
                  <>
                    <div className={`${showLyricsPanel ? 'flex-1' : 'flex-1'} flex flex-col overflow-hidden`}>
                      {view === 'arrangement' ? (
                          ...
                          <div className="flex border-b border-gray-700">
                            <div className="w-64 bg-[#252525] border-r border-gray-700 p-2 flex items-center justify-between">
                              ...
                              <div className="flex gap-1">
                                <Button size="sm" variant="ghost" className="h-6 px-2 text-xs" onClick={() => setShowAddTrackDialog(true)} data-testid="button-add-track">
                    ...
                    {showLyricsPanel && (
                      <div className="border-t border-gray-700 bg-[#1a1a1a] flex flex-col" style={{ height: '200px' }}>
                        <div className="h-8 bg-[#252525] border-b border-gray-700 flex items-center justify-between px-3">
                          <div className="flex items-center gap-2">
                            ...
                            {selectedProject && <span className="text-xs text-gray-500">- {selectedProject.title}</span>}
                          </div>
                          <Button size="sm" variant="ghost" className="h-6 w-6 p-0 text-gray-400 hover:text-white" onClick={() => setShowLyricsPanel(false)} data-testid="button-close-lyrics-panel">
                        ...
                        <div className="flex-1 p-3">
                          <Textarea
                            ...
                            placeholder={selectedProject ? "Type or paste your lyrics here..." : "Select a project to add lyrics"}
        ...
        <Dialog open={isAIMixing} onOpenChange={setIsAIMixing}>
          <DialogContent className="bg-[#252525] border-gray-700 text-white">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2"><Brain className="h-5 w-5 text-purple-400" />AI Mixing Assistant</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <p className="text-sm text-gray-300">Max Booster AI will analyze and optimize your mix automatically:</p>
              ...
              <Button 
                ...
                onClick={() => handleAIMix()} 
                disabled={aiMix.currentState === 'requesting' || aiMix.currentState === 'processing'} 
        ...
        <Dialog open={isAIMastering} onOpenChange={setIsAIMastering}>
          <DialogContent className="bg-[#252525] border-gray-700 text-white">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-blue-400" />AI Mastering Suite</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <p className="text-sm text-gray-300">Professional mastering with AI-powered optimization:</p>
              <ul className="space-y-1 text-xs text-gray-400">
              ...
              <Button 
                ...
                disabled={aiMaster.currentState === 'requesting' || aiMaster.currentState === 'processing'} 
        ...
        <Dialog open={showAnalysisPanel} onOpenChange={setShowAnalysisPanel}>
          <DialogContent className="bg-[#252525] border-gray-700 text-white max-w-2xl">
            ...
            {audioAnalysis.currentState === 'requesting' || audioAnalysis.currentState === 'processing' ? (
              ...
              <div className="space-y-6" data-testid="container-analysis-results">
                <div className="flex items-center justify-between pb-2 border-b border-gray-700">
                  ...
                  <Button
                    ...
                    disabled={audioAnalysis.currentState === 'requesting' || audioAnalysis.currentState === 'processing'}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Card className="bg-[#1a1a1a] border-gray-700 p-4">
                    <div className="space-y-2">
                      ...
                      <div className="flex items-baseline gap-2">
                        <Badge variant="secondary" className="text-lg font-bold" data-testid="badge-bpm">
                  ...
                  <Card className="bg-[#1a1a1a] border-gray-700 p-4">
                    <div className="space-y-2">
                      ...
                      <div className="flex items-baseline gap-2">
                        <Badge variant="secondary" className="text-lg font-bold" data-testid="badge-key">
                          {trackAnalysisData.musicalKey || 'C'} {trackAnalysisData.scale || 'major'}
                        </Badge>
                      </div>
                    </div>
                  </Card>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-baseline gap-2">
                        <span className="text-lg font-bold" data-testid="text-duration">
                          {Math.floor((trackAnalysisData.durationSeconds || 0) / 60)}:{String((trackAnalysisData.durationSeconds || 0) % 60).padStart(2, '0')}
                        </span>
                ...
                <div className="space-y-4">
                  ...
                  <Card className="bg-[#1a1a1a] border-gray-700 p-4">
                    <div className="space-y-2">
                      <Label className="text-xs text-gray-400">Spectral Centroid (Brightness)</Label>
                      <div className="flex items-baseline gap-2">
                      ...
                      <p className="text-xs text-gray-500">
                        {(trackAnalysisData.spectralCentroid || 1500) > 2000 ? 'Bright, high-frequency content' : 'Warm, low-frequency content'}
                      </p>
                    </div>
                  </Card>
                </div>
              ...
              <div className="flex flex-col items-center justify-center py-12 space-y-4" data-testid="container-no-analysis">
                ...
                >
                  {audioAnalysis.currentState === 'requesting' || audioAnalysis.currentState === 'processing' ? (
                    <>
                      ...
                      {audioAnalysis.currentState === 'requesting' ? 'Requesting...' : 'Analyzing...'}
                    </>
                  ) : (
                    <>
            ...
            {audioAnalysis.currentState === 'error' && (
              <Card className="bg-red-900/20 border-red-700 p-4" data-testid="card-analysis-error">
                  ...
                  <div className="space-y-2 flex-1">
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        ...
                        {audioAnalysis.retryCount >= 3 ? 'Max retries' : `Try Again (${audioAnalysis.retryCount}/3)`}
                      </Button>
                      <Button
                        size="sm"
                        variant="ghost"
            ...
            {audioAnalysis.progress > 0 && audioAnalysis.progress < 100 && !showAnalysisPanel && (
              <Card className="bg-blue-900/20 border-blue-700 p-4 mt-4" data-testid="card-analysis-progress">
                <div className="space-y-2">
                  <p className="text-sm text-blue-400">Analyzing Audio...</p>
                  <Progress value={audioAnalysis.progress} className="h-2" />
                  <p className="text-xs text-gray-400">{Math.round(audioAnalysis.progress)}% complete</p>
                </div>
              </Card>
            )}
          </DialogContent>
        ...
        <ConversionDialog
          ...
          availableFiles={recentFiles.map((f: any) => ({
        ...
        <Dialog open={showAddTrackDialog} onOpenChange={setShowAddTrackDialog}>
          <DialogContent className="bg-[#252525] border-gray-700 text-white">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2"><Plus className="h-5 w-5 text-green-400" />Add Track</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <div className="space-y-2">
                <Label>Track Type</Label>
                <Select value={newTrackType} onValueChange={(val: 'audio' | 'midi' | 'instrument') => setNewTrackType(val)}>
                  <SelectTrigger className="bg-[#1a1a1a] border-gray-700" data-testid="select-track-type">
                    <SelectValue />
                  </SelectTrigger>
        ...
        <Dialog open={showAddBusDialog} onOpenChange={setShowAddBusDialog}>
          <DialogContent className="bg-[#252525] border-gray-700 text-white">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2"><MonitorSpeaker className="h-5 w-5 text-blue-400" />Create Mix Bus</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <div className="space-y-2">
              ...
              <div className="pt-2 text-xs text-gray-400">
                <p>Mix buses allow you to group multiple tracks together for processing and routing.</p>
              </div>
              <Button className="w-full bg-blue-600 hover:bg-blue-700" onClick={() => createMixBusMutation.mutate()} disabled={createMixBusMutation.isPending || !newBusName} data-testid="button-confirm-add-bus">
