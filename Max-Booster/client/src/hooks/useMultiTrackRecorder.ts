...
export function useMultiTrackRecorder() {
  ...
  const startInputMonitoring = useCallback(async (trackId: string, trackName: string) => {
  ...
  const uploadRecordings = useCallback(async (
    ...
    for (const [trackId, blob] of recordedBlobs) {
      try {
        ...
        console.error(`Error uploading recording for track ${trackId}:`, error);
        throw error;
      }
    }
  ...
  const compensateLatency = useCallback((startPosition: number): number => {
