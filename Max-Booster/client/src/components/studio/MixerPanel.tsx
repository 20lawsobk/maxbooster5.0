...
export function MixerPanel({
  ...
  const updateEffectsMutation = useMutation({
    mutationFn: async ({ trackId, effects }: { trackId: string; effects: Partial<TrackEffects> }) => {
      if (!projectId) {
  ...
  const updateCompressor = (trackId: string, field: keyof TrackEffects['compressor'], value: number | boolean) => {
    setTrackEffects(prev => {
      const updated = {
        ...prev,
        [trackId]: {
  ...
  const updateReverb = (trackId: string, field: keyof TrackEffects['reverb'], value: number | string | boolean) => {
    setTrackEffects(prev => {
      const updated = {
        ...prev,
        [trackId]: {
                  ...
                  <CardContent className="p-4 flex flex-col gap-4">
                    ...
                    >
                      <div className="flex items-center gap-2">
                        {track.trackType === 'audio' && <Music className="w-3 h-3" style={{ color: track.color }} />}
                        {track.trackType === 'instrument' && <Mic className="w-3 h-3" style={{ color: track.color }} />}
                        <div 
                    ...
                    <AccordionContent className="space-y-4 pt-2">
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          >
                            <Label htmlFor={`eq-bypass-${track.id}`} className="text-xs" style={{ color: 'var(--studio-text-muted)' }}>
                              Bypass
                            </Label>
                            <Switch
                              id={`eq-bypass-${track.id}`}
                              checked={effects.eq.bypass}
                              onCheckedChange={(checked) => updateEQ(track.id, 'bypass', checked)}
                      ...
                      {/* Compressor Section */}
                      <div className="space-y-3 pb-3 border-b border-gray-700" data-testid={`effect-slot-${track.id}-compressor`}>
                        <div className="flex items-center justify-between">
                          <Label className="text-xs text-gray-400">Compressor</Label>
                          <div className="flex items-center gap-2">
                            <Label htmlFor={`comp-bypass-${track.id}`} className="text-xs text-gray-400">
                              Bypass
                            </Label>
                            <Switch
                              id={`comp-bypass-${track.id}`}
                              checked={effects.compressor.bypass}
                              onCheckedChange={(checked) => updateCompressor(track.id, 'bypass', checked)}
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="text-gray-300">{effects.compressor.threshold.toFixed(1)} dB</span>
                          </div>
                          <Slider
                            min={-50}
                            max={0}
                            ...
                            onValueChange={([value]) => updateCompressor(track.id, 'threshold', value)}
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="text-gray-300">{effects.compressor.ratio.toFixed(1)}:1</span>
                          </div>
                          <Slider
                            min={1}
                            max={10}
                            ...
                            onValueChange={([value]) => updateCompressor(track.id, 'ratio', value)}
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="text-gray-300">{effects.compressor.attack.toFixed(1)} ms</span>
                          </div>
                          <Slider
                            min={3}
                            max={30}
                            ...
                            onValueChange={([value]) => updateCompressor(track.id, 'attack', value)}
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="text-gray-300">{effects.compressor.release.toFixed(0)} ms</span>
                          </div>
                          <Slider
                            min={60}
                            max={600}
                            ...
                            onValueChange={([value]) => updateCompressor(track.id, 'release', value)}
                      ...
                      <div className="space-y-3" data-testid={`effect-slot-${track.id}-reverb`}>
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            <Label htmlFor={`reverb-bypass-${track.id}`} className="text-xs text-gray-400">
                              Bypass
                            </Label>
                            <Switch
                              id={`reverb-bypass-${track.id}`}
                              checked={effects.reverb.bypass}
                              onCheckedChange={(checked) => updateReverb(track.id, 'bypass', checked)}
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="text-gray-300">{(effects.reverb.mix * 100).toFixed(0)}%</span>
                          </div>
                          <Slider
                            min={0}
                            max={1}
        ...
        >
          ...
          >
            ...
            <CardContent className="p-4 flex flex-col gap-4">
              ...
              <div className="rounded overflow-hidden border" style={{ borderColor: 'var(--studio-border)' }}>
                <SpectrumAnalyzer
