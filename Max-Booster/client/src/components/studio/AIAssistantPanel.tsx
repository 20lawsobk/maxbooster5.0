...
export function AIAssistantPanel({ projectId, onApplyChanges }: AIAssistantPanelProps) {
  ...
  const { data: presetsData } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiRequest('/api/studio/ai-music/presets');
      return res.json();
    },
  });

  const { data: suggestions, refetch: refetchSuggestions } = useQuery<AISuggestion[]>({
    queryKey: ['/api/studio/ai-music/suggestions', projectId],
    queryFn: async () => {
      const mockSuggestions: AISuggestion[] = [
        {
          id: '1',
          type: 'warning',
          message: 'Track loudness is -18.2 LUFS, below Spotify target of -14 LUFS',
          confidence: 92,
          action: 'normalize',
        },
        {
          id: '2',
          type: 'info',
          message: 'Detected frequency buildup at 250Hz - consider EQ adjustment',
          confidence: 85,
        },
        {
          id: '3',
          type: 'success',
          message: 'Stereo image is well-balanced',
          confidence: 95,
        },
      ];
  ...
  const analyzeLoudnessMutation = useMutation({
    mutationFn: async (targetLUFS: string) => {
      const res = await apiRequest('/api/studio/ai-music/analyze-loudness', {
        method: 'POST',
        body: JSON.stringify({ projectId, targetLUFS }),
      });
      return res.json();
    },
  ...
  const matchReferenceMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      const res = await apiRequest('/api/studio/ai-music/match-reference', {
        method: 'POST',
        body: formData,
      });
      return res.json();
  ...
  const applyGenrePresetMutation = useMutation({
    mutationFn: async (genreId: string) => {
      const res = await apiRequest('/api/studio/ai-music/apply-genre-preset', {
        method: 'POST',
        body: JSON.stringify({ projectId, genreId }),
      });
      return res.json();
    },
    onSuccess: (data) => {
