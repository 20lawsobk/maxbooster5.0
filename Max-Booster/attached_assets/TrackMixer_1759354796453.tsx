...
export function TrackMixer() {
  ...
  const toggleMute = (trackId: string) => {
    setTracks(
      tracks.map((track) => (track.id === trackId ? { ...track, isMuted: !track.isMuted } : track))
    );
  };

  const toggleSolo = (trackId: string) => {
    setTracks(
      tracks.map((track) => (track.id === trackId ? { ...track, isSolo: !track.isSolo } : track))
    );
  };

  const updateGain = (trackId: string, value: number[]) => {
    setTracks(tracks.map((track) => (track.id === trackId ? { ...track, gain: value[0] } : track)));
  };

  const updatePan = (trackId: string, value: number[]) => {
    setTracks(tracks.map((track) => (track.id === trackId ? { ...track, pan: value[0] } : track)));
  };
        ...
        {tracks.map((track, index) => (
          <div key={track.id} className="bg-muted/20 rounded-lg p-3" data-testid={`track-${index}`}>
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium" data-testid={`text-track-name-${index}`}>
                {track.name}
              </span>
            ...
            <div className="space-y-3">
              ...
              {/* Level Meter */}
              <div
                className="h-16 bg-background rounded flex items-end justify-center space-x-1 p-1"
                data-testid={`level-meter-${index}`}
              >
                <div
