...
export class ContentAnalysisService {
  private imageClassificationModel: tf.LayersModel | null = null;
  private faceDetectionModel: tf.LayersModel | null = null;
  private textDetectionModel: tf.LayersModel | null = null;
  ...
  private async initializeModels(): Promise<void> {
    try {
      // Build lightweight custom image classification model
   ...
   */
  private buildImageClassificationModel(): tf.LayersModel {
   ...
   */
  private buildFaceDetectionModel(): tf.LayersModel {
    const model = tf.sequential({
      layers: [
        tf.layers.conv2d({
          inputShape: [224, 224, 3],
        ...
        tf.layers.dense({ units: 1, activation: 'sigmoid' }), // Binary: has faces or not
      ],
    });
   ...
   */
  private buildTextDetectionModel(): tf.LayersModel {
    const model = tf.sequential({
      layers: [
        tf.layers.conv2d({
          inputShape: [224, 224, 1], // Grayscale
          filters: 32,
          kernelSize: 3,
          activation: 'relu',
        }),
        ...
        tf.layers.flatten(),
        tf.layers.dense({ units: 64, activation: 'relu' }),
        tf.layers.dense({ units: 1, activation: 'sigmoid' }), // Binary: has text or not
      ],
    });
  ...
  private async detectFaces(imageBuffer: Buffer): Promise<{
    ...
    try {
      ...
      // Run face detection
      const prediction = this.faceDetectionModel.predict(imageTensor) as tf.Tensor;
      ...
      const count = hasFaces ? Math.ceil(hasFacesProbability * 2) : 0; // Estimate face count
