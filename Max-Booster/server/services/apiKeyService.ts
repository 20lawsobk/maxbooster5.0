 ...
 */
export async function generateApiKey(userId: string, keyName: string, tier: 'free' | 'pro' | 'enterprise' = 'free') {
  try {
    ...
    console.log(`🔑 Generated API key for user ${userId}: ${keyName} (${tier} tier, ${rateLimit} req/sec)`);
...
export async function validateApiKey(req: ApiKeyRequest, res: Response, next: NextFunction) {
  try {
    ...
    const [keyRecord] = await db
      ...
      .from(apiKeys)
      .where(and(
        eq(apiKeys.hashedApiKey, hashedApiKey),
...
export async function rateLimitApiKey(req: ApiKeyRequest, res: Response, next: NextFunction) {
  try {
    ...
    try {
      ...
      console.warn('⚠️  Rate limiting bypassed due to Redis error');
      next();
    }
  } catch (error) {
  ...
  try {
    const keys = await db
      .select({
        ...
        // SECURITY: No key preview - plaintext keys are never stored in database
      })
      .from(apiKeys)
