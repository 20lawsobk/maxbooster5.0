...
export class AdvertisingDispatchService {
   ...
   */
  async activateCampaign(campaignId: number, userId: string): Promise<{
    success: boolean;
    message: string;
    results?: {
      postsCreated: number;
    ...
    try {
      ...
      if (creatives.length === 0) {
        return {
          ...
          error: 'This campaign has no creatives to post. Please create at least one creative first.'
        };
      }
      ...
      for (const creative of creatives) {
        ...
        const content = {
          ...
          mediaUrl: creative.assetUrls && creative.assetUrls.length > 0 
        ...
        for (const result of publishResults) {
          if (result.success && result.postId) {
            ...
            try {
              ...
              console.error('Failed to create delivery log:', err);
            }
          } else {
  ...
  async collectCampaignEngagement(campaignId: number, userId: string): Promise<void> {
    try {
      ...
      if (!organicMetrics || !organicMetrics.posts) {
        console.log('No organic posts to track for campaign', campaignId);
        return;
      }
      ...
      for (const post of organicMetrics.posts) {
        ...
        try {
          ...
          console.error(`Failed to collect engagement for ${post.platform} post ${post.postId}:`, err.message);
      ...
      console.log(`✅ Updated engagement metrics for campaign ${campaignId}: ${totalImpressions} impressions, ${totalEngagements} engagements`);
      
    } catch (error: any) {
      console.error('Failed to collect campaign engagement:', error);
    }
  }
  ...
  private async createCalendarEntry(
    ...
    const entry = await db.insert(contentCalendar).values({
      userId,
      ...
      hashtags: this.extractHashtags(creative.normalizedContent || creative.rawContent || '') as any,
   ...
   */
  private calculateOrganicBoost(engagement: any): number {
  ...
  async collectAllActiveEngagement(userId?: string): Promise<void> {
    try {
      ...
      console.log(`🔄 Collecting engagement for ${activeCampaigns.length} active campaigns...`);
      ...
      console.log(`✅ Finished collecting engagement for ${activeCampaigns.length} campaigns`);
      
    } catch (error: any) {
      console.error('Failed to collect all engagement:', error);
    }
  }
