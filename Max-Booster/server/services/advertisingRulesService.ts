...
export class AdvertisingRulesService {
   ...
   */
  private shouldTrigger(rule: any, variant: any): boolean {
    ...
    switch (condition.metric) {
      ...
      case 'time':
        const hoursSinceCreated = (Date.now() - new Date(variant.createdAt).getTime()) / (1000 * 60 * 60);
        return this.compareMetric(hoursSinceCreated, condition.operator, condition.threshold);
      case 'viralityScore':
        return this.compareMetric(variant.viralityScore || 0, condition.operator, condition.threshold);
      default:
        return false;
    }
  }
   ...
   */
  private async executeRule(rule: any, variant: any): Promise<any> {
   ...
   */
  private generateTriggerReason(rule: any, variant: any): string {
    const { condition } = rule;
    const metrics = variant.actualMetrics || {};
   ...
   */
  private extractLearnings(rule: any, variant: any): string {
    ...
    if (metrics.engagement && variant.predictedEngagement) {
      ...
      if (performanceRatio < 0.5) {
        learnings.push(`Organic engagement ${Math.round((1 - performanceRatio) * 100)}% below prediction - content may not resonate with audience`);
      } else if (performanceRatio > 1.5) {
        learnings.push(`Organic engagement ${Math.round((performanceRatio - 1) * 100)}% above prediction - high-performing content, allocate more reach to similar posts`);
      }
    }
    
    // Platform-specific learnings
    learnings.push(`${variant.platform} organic performance: ${this.formatMetricSnapshot(metrics)}`);
    
    // Virality learnings
    if (variant.viralityScore) {
      if (variant.viralityScore < 50) {
        learnings.push('Low virality score - optimize with more hashtags, questions, and visual content');
    ...
    if (organicReach > 0) {
      learnings.push(`Achieved ${organicReach} organic reach with $0 ad spend - equivalent to ~$${this.estimateAdSpendEquivalent(organicReach, variant.platform)} in traditional advertising`);
    }
   ...
   */
  private async executePivot(strategy: any, variant: any): Promise<void> {
    if (!strategy) return;
    ...
    if (strategy.swapCreative) {
      ...
      console.log(`Pivot: Paused underperforming ${variant.platform} variant, recommend new creative`);
    }
  }
   ...
   */
  private formatMetricSnapshot(metrics: any): string {
    const parts: string[] = [];
   ...
   */
  private getRunDuration(variant: any): number {
