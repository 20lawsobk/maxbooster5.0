...
export class AIService {
  ...
  private async initializeInHouseAI(): Promise<void> {
    try {
      ...
      if (!redis) {
        console.warn('⚠️  AIService: Redis not available, caching disabled');
        return;
      }
  ...
  async generateSocialContent(options: SocialContentOptions): Promise<{ content: string[] }> {
    try {
      ...
      const platformData = await this.getContentStructure(platform) || await this.getContentStructure('instagram');
      if (!platformData) {
  ...
  private async analyzeAudio(audioData: Buffer): Promise<AudioAnalysisResult> {
    ...
    const genreProfile = await this.getGenreProfile(detectedGenre.toLowerCase()) || await this.getGenreProfile('electronic');
