  ...
  async (req, res) => {
    try {
      ...
      for (const event of events) {
        const { sg_message_id, email, event: eventType, timestamp: eventTimestamp, reason, smtp_response } = event;
 ...
 */
function mapSendGridEventType(eventType: string): 'delivered' | 'bounce' | 'spam' | 'unsubscribe' | 'open' | 'click' | 'deferred' | 'dropped' {
