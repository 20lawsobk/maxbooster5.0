...
router.get('/ai/forecast-revenue', async (req: Request, res: Response) => {
  try {
    ...
    if (isAdmin) {
      // Admin: Calculate platform-wide MRR
      ...
      currentMRR = (Number(revenueData?.totalRevenue) || 0) / 3; // Average monthly
      growthRate = 25; // Default 25% growth
    } else {
      // Regular user: Calculate personal revenue
...
router.post('/music/career-growth', async (req: Request, res: Response) => {
  try {
    ...
    const predictedValue = Math.round(currentValue * 1.25); // 25% growth prediction
    const growthRate = 25;
    ...
    return res.json({
      ...
      recommendations: [
        ...
        'Optimize content for playlist placement',
      ],
      confidence: 75,
    });
  } catch (error) {
...
router.get('/music/fanbase', async (req: Request, res: Response) => {
  try {
    ...
    const activeListeners = Math.round(totalFans * 0.65); // 65% active
    const engagementRate = 2.5; // 2.5% average engagement
    ...
    return res.json({
      ...
      demographics: {
        ...
        peakListeningTimes: ['Evening (6-9 PM)', 'Morning (7-9 AM)', 'Night (9-11 PM)'],
      },
      growthOpportunities: [
        'Expand presence on TikTok for younger audience',
