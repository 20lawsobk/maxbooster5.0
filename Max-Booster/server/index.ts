...
import {
  ...
  memoryMonitoring,
} from './middleware/requestCorrelation';
...
if (process.env.NODE_ENV === 'production' && typeof (global as any).gc !== 'function') {
  logger.warn('⚠️  WARNING: Production server running without --expose-gc flag');
  logger.warn('⚠️  Recommended start: NODE_ENV=production node --expose-gc dist/index.js');
  logger.warn('⚠️  This flag enables manual garbage collection for 24/7/365 operation');
  logger.warn('⚠️  Server will continue but may experience memory pressure over time');
}
...
// APM Monitoring middleware (custom metrics tracking)
...
(async () => {
  ...
  // Verify SendGrid webhook public key is configured in production
  if (process.env.NODE_ENV === 'production' && !process.env.SENDGRID_WEBHOOK_PUBLIC_KEY) {
    logger.info(
      'ℹ️  SendGrid webhook signature verification disabled (SENDGRID_WEBHOOK_PUBLIC_KEY not configured)'
    );
    logger.info(
      '   Email delivery works normally, webhook bounce tracking disabled until key is configured'
    );
  }
  ...
  setInterval(async () => {
    try {
      ...
      logger.error('Error in anomaly detection job:', error);
    }
  }, FIVE_MINUTES);
  ...
  setTimeout(async () => {
    try {
      ...
      logger.error('Error in initial anomaly detection:', error);
    }
  }, 60000);
  ...
  app.get('/api/health/metrics', (req, res) => res.json(getHealthStatus()));
  ...
  logger.info('🚀 Max Booster Platform - 24/7/365 Ready!');
  logger.info('✅ High-availability monitoring active');
  logger.info('🔄 Auto-recovery systems enabled');
  logger.info('📊 Real-time health tracking operational');
})();
